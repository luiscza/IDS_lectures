...
elements_set <- html_elements(url_p, xpath = "//h2[text()='Alphabetically']//following::div[contains(@class,'div-col')]//li/a[1]")
```
...
```{r, eval = FALSE}
library(rvest)
url_p <- read_html("https://www.spiegel.de")
...
xpath <- '//*[contains(concat( " ", @class, " " ), concat( " ", "focus\\:opacity-moderate", " " ))] | //*[contains(concat( " ", @class, " " ), concat( " ", "align-middle", " " ))]'
headlines <- html_elements(url_p, xpath = xpath) 
headlines_raw <- html_text(headlines)
length(headlines_raw)
head(headlines_raw)
headlines_raw <- str_subset(headlines_raw, "\\n\\n", negate = TRUE) # remove empty strings
```
